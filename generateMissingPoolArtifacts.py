...
def main():
    ...
    for i in range(2):
        ...
        path = os.path.join(
            "./balpy/balancer-deployments/tasks/",
            deprecated_string)
        dir_list = sorted(os.listdir(path))

        # filter out deprecated and scripts folders in the upper level folder
        dir_list = [d for d in dir_list if d not in ["deprecated", "scripts"]]

        # Filter for pool factories
        full_paths = [os.path.join(path, d) for d in dir_list]
        full_dir_paths = [
            d for d in full_paths if os.path.isdir(d) and "pool" in d.lower()
        ]
        ...
        for p in full_dir_paths:
            ...
            for f in files:
                if "pool" in f.lower() and "factory" in f.lower():
                    ...
                    # If not pool artifact exists, extract it from the pool
                    # factory's build-info
